...
/**
 * CONSOLIDAÇÃO REAL do SEPULTURA com todas as correções aplicadas
 */
async function realSepulturaConsolidation() {
  try {
    const artistPath = "/home/zegkreist/Documents/Pessoal/plex_server/music/Sepultura";

    console.log("🎵 MusicCurador - CONSOLIDAÇÃO REAL SEPULTURA");
    ...
    console.log("\n📂 Estado atual do diretório Sepultura:");
    try {
    ...
    // Configurações REAIS para Sepultura
    const options = {
      dryRun: false, // ← CONSOLIDAÇÃO FÍSICA REAL!
      skipCurated: true, // Pula já curados
      similarityThreshold: 0.88, // Threshold médio para metal (pode ter muitas versões diferentes)
      normalizeToTitleCase: true, // Title Case + [CURATED]
    };

    console.log("\n⚙️  Configurações REAIS SEPULTURA:");
    ...
    console.log("   🤘 Metal albums consolidados com precisão!");

    // Confirmação final
    console.log("\n❓ Confirma a consolidação física REAL do SEPULTURA?");
    console.log("💡 Aguardando 5 segundos... (Ctrl+C para cancelar)");

    await new Promise((resolve) => setTimeout(resolve, 5000));

    console.log("\n🚀 INICIANDO CONSOLIDAÇÃO FÍSICA REAL DO SEPULTURA...\n");
    ...
    // Relatório detalhado
    console.log("\n" + "=".repeat(80));
    console.log("📊 RELATÓRIO FINAL - CONSOLIDAÇÃO REAL SEPULTURA");
    console.log("=".repeat(80));
    ...
    if (result.consolidationResults) {
      ...
      if (successful.length > 0) {
        console.log("\n🤘 ÁLBUNS DE METAL CONSOLIDADOS COM SUCESSO:");
      ...
      try {
        ...
        if (stdout.trim()) {
          console.log("\n🏷️  ÁLBUNS SEPULTURA COM TAG [CURATED]:");
          stdout.split("\n").forEach((line, index) => {
      ...
      console.log("💡 Todos os álbuns Sepultura já estão organizados ou não há duplicatas");
    }
    ...
    console.log("\n" + "=".repeat(80));
    console.log("🤘 CONSOLIDAÇÃO REAL DO SEPULTURA CONCLUÍDA!");
    console.log("💡 Verifique o diretório Sepultura para confirmar as mudanças");
    console.log(`📂 Caminho: ${artistPath}`);
    console.log("🎸 Metal albums organizados com sucesso!");
    console.log("=".repeat(80));
  } catch (error) {
