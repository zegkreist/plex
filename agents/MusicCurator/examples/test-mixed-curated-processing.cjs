...
async function testMixedCuratedProcessing() {
  ...
  const consolidator = new AlbumConsolidator({
    allatherApiUrl: "http://localhost:3001",
    musicDirectory: "/home/zegkreist/Music",
  });

  // Artista para teste: Sepultura (sabemos que existe)
  const artistName = "Sepultura";
  const artistPath = path.join("/home/zegkreist/Music", artistName);
  ...
  try {
    ...
    const artists = await consolidator.scanMusicDirectory();
